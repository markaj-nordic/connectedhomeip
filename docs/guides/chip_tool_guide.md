...
You can find source files of the CHIP Tool in the `examples/chip-tool` directory.

> **Note:** The CHIP Tool currently only supports commissioning and remembering one device at
> a time. The configuration state is cached in the ``/tmp/chip_tool_config.ini`` file.
> Deleting this and other ``.ini`` files in the ``/tmp`` directory can sometimes resolve issues
...
1. Install all required packages for Matter and prepare the source code and the build system.
   Read the [Building Matter](BUILDING.md) guide for instructions.
...
To check if the CHIP Tool runs correctly, execute the following command from the ``BUILD_PATH`` directory:

```
$ chip-tool
```

As a result, the CHIP Tool prints all available commands.
These are called _clusters_ in this context, but not all listed
commands correspond to the _clusters_ in the Data Model (for example,
pairing or discover commands). Each listed command can however become the root of the new
more complex command by appending it with sub-commands. Examples of
specific commands and their use cases are described in the [Supported commands and options](#commands) section.
...
This section describes how to use CHIP Tool to test the Matter device. The following
steps depend on the application clusters that you implemented on the device.
...
with the Bluetooth LE commissioning. You can use other Matter examples and
still follow this procedure.
If you use a different example, the [Step 7](#step-7-control-application-data-model-clusters) may vary depending on
...
[Matter lighting-app example](https://github.com/project-chip/connectedhomeip/tree/master/examples/lighting-app) documentation.
...
To follow the next steps, the Thread or Wi-Fi network must be up and running. For
...
You must provide the CHIP Tool with network credentials that will be
used in the device commissioning procedure to configure the device with a
network interface, such as Thread or Wi-Fi.

The Matter specification does not define how the Thread or Wi-Fi credentials are to be
...
You must get the following Wi-Fi network credentials to commission the
Matter device to the Wi-Fi network in the following steps:

-   Wi-Fi SSID
-   Wi-Fi password

The steps required to determine the SSID and password may vary
...
Before communicating with the Matter device, first have it join the existing Thread or Wi-Fi network.
Depending on your network setup, this can be done over Bluetooth LE or IP.

#### Commissioning over Bluetooth LE

This section describes how your device can join the existing Thread or Wi-Fi network over Bluetooth LE and then be commissioned into a Matter network.
...
This log will mean that the PASE (Password-Authenticated Session Establishment) session
using SPAKE2+ protocol has been established.
...
-   _<operational_dataset\>_ is the Operational Dataset determined in the step 3.
...
If you prefer the hexadecimal format, use the ``hex:`` prefix.
For example:
...
> **Note:**
> The _<node_id>_ can be provided as a hexadecimal value with the ``0x`` prefix.

#### Commissioning into a network over IP

This option is available when the Matter device is already present in an IP network (Thread or Wi-Fi enabled),
but it has not been commissioned to a Matter network yet.

##### Commissioning with setup PIN code

Use the following command pattern to discover devices and try to pair with the first discovered one
using the provided setup code:
...
-   _<node_id\>_ is the user-defined ID of the node being commissioned.
-   _<pin_code\>_ is device specific _setup PIN code_ determined in the step 4 and used to discover the device.

##### Commissioning with long discriminator

Use the following command pattern to discover devices with a long discriminator and try to pair
with the first discovered one using the provided setup code.
...
Use the following command pattern to discover devices based on the given QR code payload
and try to pair with the first discovered one:
...
-   _<node_id\>_ is the user-defined ID of the node being commissioned.
-   _<qrcode_payload\>_ is the QR code payload ID, for example ``MT:Y.K9042C00KA0648G00``.
...
In this command, _<node_id\>_ is the user-defined ID of the node which is going to be forgotten
by the CHIP Tool.
...
commissioned to the network.
You can now test the device by interacting with Data Model
clusters.

For instance, in case of the lighting app example, the application has ``onoff`` and ``levelcontrol`` clusters
implemented. This means that you can test by toggling the bulb (using ``onoff``) or manipulating its
brightness (using ``levelcontrol``).
...
attributes that a controller can obtain from a device.
These attributes can include the vendor name,
the product name, or the software version.

Use the CHIP Tool's ``read`` command on the ``basic`` cluster to read those values from the device:
...
Message tracing allows to capture CHIP Tool secure messages that can be used
for test automation.
...
The CHIP Tool allows to run a set of tests, already compiled in the tool, against
a paired Matter device.
...
The CHIP Tool offers a utility for parsing the Matter onboarding setup payload and prnting it in
a readable form. For example, the payload can be printed on the device console
during boot.

To parse the setup code, use the ``payload`` command with the ``parse-setup-payload``
...
Use the following command to print the available sub-commands of
...
To resolve the DNS-SD name corresponding with the given Node ID and update the address
of the node in the device controller, use the following command pattern:
...
As mentioned in the [Using the CHIP Tool for Matter device testing](#using) section,
executing the ``chip-tool`` command with a particular cluster name lists all operations
supported for this cluster, as in the following command pattern:
...
According to this list, the ``binding`` cluster supports operations such as
...
instance when the attribute value is changed or a specific event happens.
...
The Access Control List (ACL) concept allows to govern all Data Model interactions (such as read
attribute, write attribute, invoke command). For more information about ACL, see
...
to receive commands from the bonded device. After the binding process, the bonded device
contains information about connected device, such as IPv6 address and the route to
the endpoint in the Matter network. This allows to send command directly from the bonded
device to the end device without using the border router as a proxy.
